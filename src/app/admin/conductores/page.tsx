...
const ConductoresManager: React.FC = () => {
  ...
  const fetchConductores = async () => {
    try {
      const response = await fetch('https://api.rutasegura.xyz/users/drivers');
  ...
  const handleCreate = async (newConductor: Omit<Conductor, '_id'>) => {
    try {
      const response = await fetch('https://api.rutasegura.xyz/users/create', {
        method: 'POST',
  ...
  const handleUpdate = async (updatedConductor: Conductor) => {
    try {
      const response = await fetch(`https://api.rutasegura.xyz/users/users/${updatedConductor._id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (result.isConfirmed) {
        const response = await fetch(`https://api.rutasegura.xyz/users/users/${id}`, {
          method: 'DELETE',
        });
