...
const Tabla: React.FC = () => {
  ...
  const fetchVehiculos = async () => {
    try {
      setIsLoading(true);
      const response = await fetch("http://35.153.187.71/unidades");
      if (!response.ok) {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (result.isConfirmed) {
        const response = await fetch(`http://35.153.187.71/unidades/unidad/${id}`, {
          method: 'DELETE',
        });
...
const handleSubmit = async (unidad: Omit<Unidad, '_id' | 'estado'>) => {
  try {
    let response;
    if (unidadToEdit) {
      response = await fetch(`http://35.153.187.71/unidades/unidad/${unidadToEdit.placa}`, {
        method: 'PUT',
        headers: {
      ...
      response = await fetch("http://35.153.187.71/unidades", {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
